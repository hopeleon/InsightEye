...
class RealtimeSessionStore:
    ...
    def status(self, session_id: str) -> dict[str, Any]:
        ...
        if session.get("rolling_disc_analysis") is None and (session["segments"] or session.get("partial_transcripts")):
            self._schedule_disc_analysis(session_id)
        return session
    ...
    def _schedule_disc_analysis(self, session_id: str) -> None:
        with self._analysis_task_lock:
            task = self._analysis_tasks.get(f"disc:{session_id}")
            if task and not task.done():
                return
            ...
            async def _run() -> None:
                try:
                    ...
                    with self._analysis_task_lock:
                        self._analysis_tasks.pop(f"disc:{session_id}", None)
            ...
            self._analysis_tasks[f"disc:{session_id}"] = loop.create_task(_run())
