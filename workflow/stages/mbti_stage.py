...
def _analyze_e_i(features: dict, transcript: str, knowledge: dict) -> dict:
    ...
    # 最小信号保护：总分低于阈值时判定为 neutral，避免无特征文本强行输出偏好
    if (e_score + i_score) < 3:
        preference = "neutral"
        strength = 50
        confidence = "low"
    elif e_percentage > 65:
        preference = "E"
        strength = e_percentage
        confidence = "high" if e_percentage > 75 else "medium"
    elif i_percentage > 65:
        preference = "I"
        strength = i_percentage
        confidence = "high" if i_percentage > 75 else "medium"
    else:
        preference = "neutral"
        strength = 50
        confidence = "low"
...
def _analyze_n_s(features: dict, transcript: str, knowledge: dict) -> dict:
    ...
    # 最小信号保护：总分低于阈值时判定为 neutral
    if (n_score + s_score) < 3:
        preference = "neutral"
        strength = 50
        confidence = "low"
    elif n_percentage > 65:
        preference = "N"
        strength = n_percentage
        confidence = "high" if n_percentage > 75 else "medium"
    elif s_percentage > 65:
        preference = "S"
        strength = s_percentage
        confidence = "high" if s_percentage > 75 else "medium"
    else:
        preference = "neutral"
        strength = 50
        confidence = "low"
...
def _analyze_t_f(features: dict, transcript: str, knowledge: dict) -> dict:
    ...
    # 最小信号保护：总分低于阈值时判定为 neutral
    if (t_score + f_score) < 3:
        preference = "neutral"
        strength = 50
        confidence = "low"
    elif t_percentage > 65:
        preference = "T"
        strength = t_percentage
        confidence = "high" if t_percentage > 75 else "medium"
    elif f_percentage > 65:
        preference = "F"
        strength = f_percentage
        confidence = "high" if f_percentage > 75 else "medium"
    else:
        preference = "neutral"
        strength = 50
        confidence = "low"
...
def _analyze_j_p(features: dict, transcript: str, knowledge: dict) -> dict:
    ...
    # 最小信号保护：总分低于阈值时判定为 neutral
    if (j_score + p_score) < 3:
        preference = "neutral"
        strength = 50
        confidence = "low"
    elif j_percentage > 65:
        preference = "J"
        strength = j_percentage
        confidence = "high" if j_percentage > 75 else "medium"
    elif p_percentage > 65:
        preference = "P"
        strength = p_percentage
        confidence = "high" if p_percentage > 75 else "medium"
    else:
        preference = "neutral"
        strength = 50
        confidence = "low"
